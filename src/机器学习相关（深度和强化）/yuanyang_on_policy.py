...
class Yuanyang:
    ...
    def q_returns_N_init(self):
        ...
        for state in self.states:
            if state in self.states_terminal:
                self.pi_a_st[state] = None
                continue
            self.pi_a_st[state] = [0.25,0.25,0.25,0.25]
    ...
    # ['up','down','left','right'],按照概率随机选择一个，并返回选择的动作
    def rand_pick_action(self, seq, probabilities):
        x = random.random()   # 随机生成一个实数，它在[0, 1)范围内
        cumprob = 0.0
        for action , pi in zip(seq , probabilities):
            cumprob += pi
            if x < cumprob: 
                break
        return action
    ...
    def generate_episode(self, state_0, action_0):
        ...
        episode = {0: [state_0, action_0, reward]}
        
        i = 1
        while(flag == 2):
            # epsilon-greedy选择下一个行为
            self.epsilon_greedy(next_state)
            next_action = self.rand_pick_action(self.actions,self.pi_a_st[next_state])    #策略!!!!!!!!!!!!!!!!!!!!!!!
    ...
    def epsilon_greedy(self, state):
        ...
        # 求Π(a|st)
        for action in self.states_actions[state]:
            if action == max_value_action:
                a0[action] = 1 - self.epsilon + self.epsilon / 4
            else:
                a0[action] = self.epsilon / 4
        a1[0] = a0['up']
        a1[1] = a0['down']
        a1[2] = a0['left']
        a1[3] = a0['right']
        # 更新策略
        self.pi_a_st[state] = a1

    def MC_On_Policy(self, episode):
    ...
    def MC_On_Policy_improvement(self):
        # 改善策略
        for state in self.states:   
            if state in self.states_terminal:
                self.pi_a_st[state] = None
    ...
    # 获得每个状态下值最大的动作
    def get_states_policy_action(self):
        for state in self.states:
            max_value = 0
            flag = 0
            if state in self.states_terminal:
                continue
            if np.argmax(self.pi_a_st[state]) == 0:
                self.states_policy_action[state] = 'up'
            if np.argmax(self.pi_a_st[state]) == 1:
                self.states_policy_action[state] = 'down'
            if np.argmax(self.pi_a_st[state]) == 2:
                self.states_policy_action[state] = 'left'
            if np.argmax(self.pi_a_st[state]) == 3:
                self.states_policy_action[state] = 'right'
    ...
    def run(self): 
        for i in range(20000):
            ...
            self.epsilon_greedy(init_state)
            init_action = self.rand_pick_action(self.actions, self.pi_a_st[init_state])
            # print(random_state,random_action)
            # 生成一个episode
            episode = self.generate_episode(init_state,init_action)
            # print(episode)
            # 策略评估+改善
            self.MC_On_Policy(episode)
            self.MC_On_Policy_improvement()
        # 获得每个状态下的最佳动作
        self.get_states_policy_action()
